...
#define NUM_BITS_PER_RRMENTRY 8
#define NUM_BITS_PER_EPOCH (NUM_BITS_PER_RRMENTRY - 2)
#define NUM_BITS_PER_SUBEPOCH (NUM_BITS_PER_RRMENTRY - 2)
            ...
            if (ptr->list) {
               if (ptr->list->id > (induction_var/NUM_VERTICES_PER_EPOCH)) {
                  ...
                  if (ptr->list->sub_id < (induction_var_offset/num_vertices_per_subepoch)) {
                     if (ptr->list->next) {
                        if (ptr->list->next->id == 1+(induction_var/NUM_VERTICES_PER_EPOCH)) {
                           if (ptr->list->next->id > maxdist) {
                              maxdist = ptr->list->next->id;
                              index = i;
                           }
                        }
                        else if (2+(induction_var/NUM_VERTICES_PER_EPOCH) > maxdist) {
                           maxdist = 2+(induction_var/NUM_VERTICES_PER_EPOCH);
                           index = i;
                        }
                     }
                  }
